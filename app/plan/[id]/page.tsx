...
export default function PlanPage() {
  ...
  useEffect(() => {
    const planData = sessionStorage.getItem('fretpath_plan')
    if (planData) {
        ...
        <div className="space-y-3 mb-8">
          {plan.days.map((day) => (
            <div key={day.day_number} className="bg-white rounded-xl border border-neutral-200 overflow-hidden">
              <button
                onClick={() => setOpenDay(openDay === day.day_number ? 0 : day.day_number)}
                className="w-full flex items-center justify-between px-5 py-4 text-left"
              >
                <div className="flex items-center gap-3">
                  <div className="w-7 h-7 rounded-full bg-[#1E2A3A] text-[#D4890A] text-xs font-bold flex items-center justify-center flex-shrink-0">
                    {day.day_number}
                  </div>
                  <div>
                      ...
                      <div key={i} className="border border-neutral-100 rounded-lg p-4">
                        <div className="flex items-start justify-between gap-2 mb-2">
                          ...
                          <div className="flex items-center gap-2 flex-shrink-0">
                            <span className={`text-xs px-2 py-0.5 rounded border font-medium ${categoryColors[exercise.category] || 'bg-neutral-50 text-neutral-600 border-neutral-200'}`}>
                              {exercise.category}
                            </span>
                        ...
                        <div className="bg-[#D4890A]/8 rounded p-2">
